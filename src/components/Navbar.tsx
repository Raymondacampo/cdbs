...
import CDBS_w from "../../public/images/dashboard/CDBS_w.png";
...
export default function Navbar() {
  ...
  return (
    <header className="bg-pink text-white top-0 z-50 shadow-xs">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between h-16">
        {/* Logo */}
        <Link href="/" className="font-[Quicksand,sans-serif] font-bold text-xl text-pink tracking-wide">
          <Image src={CDBS_w} alt="CDBS Logo" className="w-40 md:w-48 h-auto" />
        </Link>
        ...
        <nav className="flex flex-col gap-1 p-6">
          {links.map((l) => (
            <Link
              ...
              className="py-3 px-2 text-2xl font-medium hover:text-pink transition-colors duration-200"
            >
              {l.label}
            </Link>
          ))}
